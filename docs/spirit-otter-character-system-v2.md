...
图中“深汐”和“拾岸”是同一角色的内部表达姿态，不是两个账号、两段关系或两套用户画像。人格路由自动发生；首版记忆依据进入研究时的全局数据同意静默写入，并由确定性规则守门。行动保存和回访创建仍必须逐项获得用户授权。
...
- 用户明确拒绝建议：深汐锁定两轮。
- 用户明确请求整理，且风险允许：进入 `blend_to_shore`。
...
首版长期记忆不逐条询问：它只在用户已接受全局研究数据同意后自动、静默写入，最长 30 天，并受到证据、敏感度、重要度、置信度与高风险 Guard 的共同限制。外部研究扩大前必须提供记忆查看、修改、关闭召回和删除能力。
...
不要替用户说话或虚构用户行为。不要索取关注、表现孤独、声称等待用户、贬低现实关系或暗示只有你理解用户。未经授权，不保存行动、建立回访或调用外部工具。长期记忆只能由独立的受控提取器提出候选，你不得在回复生成中自行写入或修改记忆。
```
...
- 类型：全局研究同意下，由受控提取器静默激活
- 内容：只保存具有当前消息逐字证据并通过确定性 Guard 的关系里程碑或支持约定。不复制行动和回访，不保存猜测性人格标签、永久脆弱点、诊断、高风险内容或依赖性判断。
...
- 行动和回访未经逐项授权创建率为 0；记忆在无全局研究数据同意或未通过 Guard 时创建率为 0。
...
4. 删除 ModeTransition；行动与回访保留逐项授权，记忆使用全局同意与确定性 Guard。
