...
import { conventionalSunSign, knowledgeForPrompt, mentionedSigns, parseMonthDay } from "./knowledge.js";

export const ASTROLOGY_SKILL_VERSION = "astrology-skill-v1";
...
function resolution(input: Parameters<TopicSkillDefinition["resolve"]>[0]): SkillResolution {
  ...
  const promptContext = [
    ...
    "星座只能作为流行文化或自我观察的谈资，不是科学诊断、人格定论或命运预测。使用‘常见说法、可以当作一个观察角度’，允许用户不同意。",
...
function fallback(input: { text: string; resolution: SkillResolution }): string {
  ...
  if (signs.length) {
    ...
    return `${details}。把这些当成聊天和自我观察的角度就好，不必拿它给谁定型。`;
  }
